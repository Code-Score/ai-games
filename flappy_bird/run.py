...
def flappygame(player): 
	...
	while True: 
		...
		for pipe in up_pipes:
			new_pipe = {'x': pipe['x'] - game_images['pipeimage'][0].get_width(), 
						'y': pipe['y'] + game_images['pipeimage'][0].get_height()}
			send_up_pipes.append(new_pipe)
		...
		for pipe in down_pipes:
			new_pipe = {'x': pipe['x'] - game_images['pipeimage'][0].get_width(), 
						'y': pipe['y']}
			send_down_pipes.append(new_pipe)
			
		move = player.update_state(**{
				'up_pipes': send_up_pipes, 
				'down_pipes': send_down_pipes, 
				'x': horizontal, 
				'y': vertical,
				'bird_height': game_images['flappybird'].get_height(), 
				'bird_width': 2*game_images['flappybird'].get_width(), 
				'pipe_height': game_images['pipeimage'][0].get_height(), 
				'pipe_width': 2*game_images['pipeimage'][0].get_width()
			}
		)
        ...
        # Fetching the digits of score. 
		numbers = [int(x) for x in list(str(player.generation))] 
		width = 0
	...
	while True: 
		...
		while True: 
			for event in pygame.event.get(): 
				...
				elif (event.type == KEYDOWN and (event.key == K_SPACE or event.key == K_UP)) or player.can_start(): 
					flappygame(player) 
