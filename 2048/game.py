...
pygame.time.set_timer(FIRE_EVENT, 100)  # 1000 milliseconds is 1 seconds.

def winCheck(board, status, theme, text_col, player):
    """
...
def playGame(theme, difficulty):
    ...
    while player.can_start():
        ...
        while True:
            ...
            key = player.update_state(board = board)
            ...
            if new_board != board:
                ...
                (board, status) = winCheck(board, status, theme, text_col, player)