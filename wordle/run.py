...
while player.can_start():
    ...
    while True:
        ...
        current_guess_from_player = player.update_state(last_results = result)

        if not(len(current_guess_from_player) == 5 and current_guess_from_player.lower() in WORDS):
            continue

        for key_pressed in current_guess_from_player.upper():
            if key_pressed in "QWERTYUIOPASDFGHJKLZXCVBNM" and key_pressed != "":
                assert(len(current_guess_string) < 5)
                create_new_letter()
        
        if len(current_guess_string) == 5 and current_guess_string.lower() in WORDS:
            result = check_guess(current_guess)
        else:
            while len(current_guess_string) > 0:
                delete_letter()
            result = []
        
        if game_result != "":
            if game_result == "W":
                player.update_score(guesses_count)
            else:
                assert(game_result == "L")
